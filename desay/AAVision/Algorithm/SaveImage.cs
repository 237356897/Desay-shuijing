  ...
  public  class SaveImage
    {
        ...
        public static void SaveResult(HWindow window,string process)
        {
            string dicpath = System.Windows.Forms.Application.StartupPath;
            string _filePath = dicpath + $"{ @"./ImageResult"}";
            string filename = DateTime.Now.Year.ToString() + "_"
                + DateTime.Now.Month.ToString() + "_"
                + DateTime.Now.Day.ToString() + "_"
                + DateTime.Now.DayOfWeek.ToString() + "_"
            ...
            { Directory.CreateDirectory(_filePath); }
            //HOperatorSet.WriteImage(Image, "jpg", 0, AppDomain.CurrentDomain.BaseDirectory + "//glueImage//" +　filename);
            HOperatorSet.WriteImage(Image, "bmp", 0, $"{_filePath }//{ DateTime.Now.ToString("yy_MM_dd_HH_mm_ss_fff_")}{process}.jpg");
        }
